...
time = c('2010-01-01T00:00:00Z','2010-01-01T00:00:00Z') #Sets time to 2040 decade
...
dataset_idTemp <-"thetao_baseline_2000_2019_depthmax"
dataset_idSIThick <- "sithick_baseline_2000_2020_depthsurf"
dataset_idDO <- "o2_baseline_2000_2018_depthmax"
dataset_idCHL <- "chl_baseline_2000_2018_depthsurf" 
dataset_idSeaVel <- "sws_baseline_2000_2019_depthmax"

#Download max benthic temp prediction  
Temp2010  <- download_layers(dataset_idTemp, 
                                variables = BenthicTempVar, 
                                constraints = constraints)
#Download max sea ice thickness prediction
SeaIceThickness2010  <- download_layers(dataset_idSIThick, 
                                        variables = SeaIceThicknessVar, 
                                        constraints = constraints)
#download mean dissolved o2 prediction
DO2010  <- download_layers(dataset_idDO, 
                               variables = o2Var, 
                               constraints = constraints)
#currents
SeaVel2010 <- download_layers(dataset_idSeaVel,
                                 variables = SeaVelVar,
                                 constraints = constraints)
#chlorophyll lt_max
Chlorophyll2010 <- download_layers(dataset_idCHL,
                                  variables = ChlorVar,
                                  constraints = constraints)
...
#un-log transform data
values(Pollock2040Predicted)[!is.na(values(Pollock2040Predicted))] <- 
  exp(predict(rf$finalModel, newdata = na.omit(regiondata)))

Pollock2040Predicted <- terra::rotate(Pollock2040Predicted)
e <- ext(-200, -150, 52, 64)
Pollock2040Predicted <-crop(Pollock2040Predicted , e)
#Plot the whole prediction map
...
ggplot() + geom_spatraster(data = Pollock2040Predicted) + 
  labs(title = "2040 Predicted Pollock Weight Catch per Unit effort (kg/ha)")+ 
  colorscheme+
theme_bw()
...
gg2040 <- ggplot() + geom_spatraster(data = Pollock2040Predicted)+
  colorscheme + labs(title = "2040 Predicted (SSP3-7.0)") +theme_bw()
#saving image to folder "Figures"
png("Figures/PollockMap2010_2040.png", width = 800, height = 400)
...
ggChange <- ggplot() + geom_spatraster(data = Pollock2040Predicted - Pollock2010Predicted)+
  colorscheme + labs(title = "Change in Predicted CPUE (2040 - 2010)", caption = "Catch per Unit Effort (CPUE) measured in catch of pollock (kg) per hectare trawled") +theme_bw()
#saving image
png("Figures/PollockMapChange.png", width = 800, height = 400)
ggChange
dev.off()
```
