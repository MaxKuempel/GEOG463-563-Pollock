...
Pollock2010Predicted <- o22010_pred
...
for (i in 1:length(LayerList)) {
 layer = biooracle_download(LayerListPred[[i]], 
                     ...
                     latitude_pred, 
                     longitude_pred)
  assign(paste0(names(LayerListPred[i]),as.character(year_to_predict),
                "_pred"), layer )
  remove(layer) #gets rid of last duplicate layer
  LayerNamesPred <-
...
#Creates copy of Bio-ORACLE raster then overwrites data with model predictions
#Assigns a new object to one of the Bio-ORACLE layers. Ensures that all spatial information is maintained. 
PollockPred <- get(paste0(names(LayerListPred[1]),as.character(year_to_predict),
                          "_pred"))
...
png(paste0("Figures/PollockMap",year_to_predict, ".png"), width = 600, height = 400)    
...
```{r DIffMap}
Change <- PollockPred - Pollock2010Predicted
